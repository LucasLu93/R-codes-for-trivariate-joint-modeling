...
n2<-2000 # number of subjects
n3<-200 # number of families
m<-10 # maximum number of observed times for each subjects    
id<-1:n2
id.2<-rep(id,each=m)

# random effects
sd.Y.R<-0.4 # related to recurrent events
var.Y.R<-sd.Y.R^2
sd.Y.L<-0.6 # related to longitudinal outcomes
var.Y.L<-sd.Y.L^2
cor.Y<-0.6
cov.Y<-cor.Y*sqrt(var.Y.R*var.Y.L)
vcov.Y<-matrix(c(var.Y.R,cov.Y,cov.Y,var.Y.L),2,2)

sd.Z.R<-0.3 # related to recurrent events
var.Z.R<-sd.Z.R^2
sd.Z.L<-0.5 # related to longitudinal outcomes
var.Z.L<-sd.Z.L^2
cor.Z<-0.4
...
beta<-c(2.4,-0.4)
xi<--0.6
lambda.d<-0.6 # shape for weibull distribution
rho.d<-5 # scale for weibull distribution

# recurrent event
gamma<-c(0.3,-0.3)
alpha<-0.6
lambda.r<-1.2 # shape for weibull distribution
rho.r<-0.3 # scale for weibull distribution
...
tau.l<-1.7
tau.z<-1.4
varphi<-4.1 # dispersion parameter

# fixed effects
true.fixed<-c(chi,theta,gamma,beta)

# hyperparameters
true.hyper<-c(rho.r,rho.d,varphi,lambda.r,lambda.d,sd.Y.R,cor.Y,sd.Y.L,sd.Z.R,cor.Z,sd.Z.L,alpha,omega.l,omega.z,tau.l,tau.z,xi)
...
for (l in 1:n.iteration){
    ...
    inla.model<-try(inla(formula, family = c("binomial","zeroinflatednbinomial0","weibullsurv","weibullsurv"),data = data.f,
                         control.family = list(list(),list(hyper = list(prob = list(initial = -10,fixed = TRUE))),list(variant=1),list(variant=1)),control.inla = list(int.strategy="eb", cmin = 0), safe = TRUE),
                    silent=TRUE)
